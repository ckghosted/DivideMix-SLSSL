...
parser.add_argument('--id', default='slssl2')
...
parser.add_argument('--tch_model_1', default='', type=str, help='name of the saved teacher model')
parser.add_argument('--tch_model_2', default='', type=str, help='name of the saved teacher model')
...
def test(epoch,net1,log,prefix='| Test',net2=None):
    net1.eval()
    if not net2 is None:
        net2.eval()
    correct1 = 0
    ...
    if not net2 is None:
        ...
        print(prefix + " Epoch #%d\t Accuracy1: %.2f%%\n" %(epoch,acc1))  
        print(prefix + " Epoch #%d\t Accuracy2: %.2f%%\n" %(epoch,acc2))  
...
def reweighting(mentor_net, dataloader):
    ...
    for ep in range(args.n_rw_epoch):
        ...
        plt.plot(x, y_clean)
        plt.plot(x, y_noisy)
        plt.savefig(os.path.join(density_path, 'ep%02d.png' % ep))
        plt.close()
        print('time elapsed:', time.time() - start)
    return prob

def eval_train_acc(model,prefix='| Train'):
    model.eval()
    correct = 0
    total = 0
    with torch.no_grad():
...
loader = dataloader.cifar_dataloader(args.dataset,r=args.r,noise_mode=args.noise_mode,batch_size=args.batch_size,num_workers=8,\
...
for epoch in range(args.num_epochs+1):   
    ...
    if epoch<args.warm_up:       
        ...
        if args.warm_up > 0 and args.num_ssl > 0:
            # student <- teacher
            for param, param_tch in zip(net1.parameters(),tch_net1.parameters()):
                param.data.copy_(param_tch.data)
            if args.cotrain:
                for param, param_tch in zip(net2.parameters(),tch_net2.parameters()):
                    param.data.copy_(param_tch.data)
        elif os.path.exists(tch_path_1):
            loaded_model = torch.load(tch_path_1)
            print('load model from %s' % tch_path_1)
            net1.load_state_dict(loaded_model['state_dict'])
            if args.cotrain and os.path.exists(tch_path_2):
                loaded_model = torch.load(tch_path_2)
                print('load model from %s' % tch_path_2)
                net2.load_state_dict(loaded_model['state_dict'])
        if args.cotrain:
            test(epoch,net1,test_log,'| (debug) Test',net2)
        else:
            test(epoch,net1,test_log,'| (debug) Test',None)  
        ...
        if args.n_rw_epoch > 0:
            if epoch == args.warm_up:       
                start_epoch = time.time()
                prob1_rw = reweighting(net1, warmup_trainloader)
                print('(the whole reweighting) time elapsed:', time.time() - start_epoch)
                if args.cotrain:
                    start_epoch = time.time()
                    prob2_rw = reweighting(net2, warmup_trainloader)
                    print('(the whole reweighting) time elapsed:', time.time() - start_epoch)
    ...
    if args.cotrain:
        test(epoch,net1,test_log,'| Test',net2)  
        if args.num_ssl > 0 and epoch < args.warm_up:
            test(epoch,tch_net1,test_tch_log,'| tch Test',tch_net2)  
    else:
        test(epoch,net1,test_log,'| Test',None)  
        if args.num_ssl > 0 and epoch < args.warm_up:
            test(epoch,tch_net1,test_tch_log,'| tch Test',None)  
