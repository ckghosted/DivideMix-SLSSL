...
parser.add_argument('--lr', '--learning_rate', default=0.0002, type=float, help='initial learning rate')
parser.add_argument('--alpha', default=0.5, type=float, help='parameter for Beta')
parser.add_argument('--lambda_u', default=0, type=float, help='weight for unsupervised loss')
parser.add_argument('--p_threshold', default=0.5, type=float, help='clean probability threshold')
parser.add_argument('--T', default=0.5, type=float, help='sharpening temperature')
parser.add_argument('--num_epochs', default=80, type=int)
parser.add_argument('--id', default='add')
...
parser.add_argument('--ep_start_rw', default=40, type=int, help='number of epochs to start M-step')
...
if args.n_rw_epoch > 0:
    ...
    if not args.ep_start_rw == 40:
        log_name = log_name + 'start%d' % args.ep_start_rw
...
for epoch in range(args.num_epochs):
    ...
    if args.cotrain:
        ...
        print('(the whole eval_train) time elapsed:', time.time() - start_epoch)
    # (2) M-step
    if epoch >= args.ep_start_rw and args.n_rw_epoch > 0:
        if epoch % args.n_epoch_per_rw == 0:
            ...
            for ii in range(10):
                ...
                print(paths_rw1[ii])
                print()
            prob1_rw_mean = np.mean(prob1_rw)
            print('before normalization, prob1_rw_mean=%.4f' % prob1_rw_mean)
            prob1_rw /= prob1_rw_mean
            prob1_rw_mean = np.mean(prob1_rw)
            print('after normalization, prob1_rw_mean=%.4f' % prob1_rw_mean)
            prob1 = args.prob_combine_r * prob1 + (1 - args.prob_combine_r) * prob1_rw
            if args.cotrain:
                prob2_rw_mean = np.mean(prob2_rw)
                print('before normalization, prob2_rw_mean=%.4f' % prob2_rw_mean)
                prob2_rw /= prob2_rw_mean
                prob2_rw_mean = np.mean(prob2_rw)
                print('after normalization, prob2_rw_mean=%.4f' % prob2_rw_mean)
