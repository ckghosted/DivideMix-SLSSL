...
parser.add_argument('--id', default='gmm2')
...
parser.add_argument('--T_rw', default=10.0, type=float, help='sharpening temperature for sample weights')
parser.add_argument('--diag_multi', default=100.0, type=float, help='diagonal loss multiplication for sample reweighting')
...
for epoch in range(args.num_warmup, args.num_epochs):
    ...
    if epoch >= args.rw_start_epoch and args.n_rw_epoch > 0:
        if epoch % args.n_epoch_per_rw == 0:
            ...
            prob2 = np.empty(50000)
            for i in range(args.num_class):
                idx_for_i = np.where(np.array(noise_label)==i)
                prob_rw_i = prob_rw[idx_for_i]
                prob_rw_i = prob_rw_i.reshape(-1,1)
                print('prob_rw_i.shape:', prob_rw_i.shape)
                gmm = GaussianMixture(n_components=2,max_iter=10,tol=1e-2,reg_covar=5e-4)
                gmm.fit(prob_rw_i)
                prob2_i = gmm.predict_proba(prob_rw_i)
                prob2_i = prob2_i[:,gmm.means_.argmax()]
                print('prob2_i.shape:', prob2_i.shape)
                prob2[idx_for_i] = prob2_i
    else:
        start_epoch = time.time()
